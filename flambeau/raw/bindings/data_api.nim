...
type
  ...
  TensorTransform*[Target]
    ...
    = object of Transform[Example[RawTensor, Target], Example[RawTensor, Target]]
...
func get*(dataset: Mnist, index: int): Example[RawTensor, RawTensor] {.importcpp: "#.get(#)".}
func is_train*(): bool {.importcpp: "#.is_train()".}
func images*(dataset: Mnist): lent RawTensor {.importcpp: "#.images()".}
  ## Returns all images stacked into a single tensor
func targets*(dataset: Mnist): lent RawTensor {.importcpp: "#.targets()".}
...
func start*(dl: StatelessDataLoader
       ): TorchDataIterator[Example[RawTensor, RawTensor]]
  {.importcpp: "#.begin()".}
  ## Start an iterator
...
iterator items*(dl: StatelessDataLoader or CppUniquePtr[StatelessDataLoader]): Example[RawTensor, RawTensor] =
...
iterator pairs*(dl: StatelessDataLoader or CppUniquePtr[StatelessDataLoader]): tuple[index: int, value: Example[RawTensor, RawTensor]] =
  # TODO: lent Example[Tensor, Tensor]
  #   borrow checker complains about 'cur' escaping it's frame
  #   but `cur.get()` already returns a borrowed view
  var cur = dl.start()
